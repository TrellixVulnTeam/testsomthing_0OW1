...
@Dark.on(admin_cmd(pattern="cbutton"))  # pylint:disable=E0602
async def _(event):
    ...
    if Config.DARKWEB_ID is None:
        await event.edit(
            "need to have a `DARKWEB_ID` for this module to work"
        )
        return
    ...
    if reply_message.media is not None:
        ...
        tgbot_reply_message = await tgbot.get_messages(
            entity=Config.DARKWEB_ID, ids=message_id_in_channel
        )
        tgbot_reply_message = tgbot_reply_message.media

    await tgbot.send_message(
        entity=Config.DARKWEB_ID,
