...
@Dark.on(admin_cmd(pattern="create (b|g|c) (.*)"))  # pylint:disable=E0602
@Dark.on(sudo_cmd(pattern="create (b|g|c) (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="link(?: |$)(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="link(?: |$)(.*)", allow_sudo=True))
