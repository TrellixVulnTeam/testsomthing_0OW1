...
@Dark.on(admin_cmd(pattern="inviteall ?(.*)"))
@Dark.on(sudo_cmd(pattern="inviteall ?(.*)", allow_sudo=True))
...
@Dark.on(admin_cmd(pattern="invite ?(.*)"))
@Dark.on(sudo_cmd(pattern="invite ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
