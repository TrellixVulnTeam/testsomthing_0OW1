...
@Dark.on(admin_cmd(pattern="dns (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="dns (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="url (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="url (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@Dark.on(admin_cmd(pattern="unshort (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="unshort (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
