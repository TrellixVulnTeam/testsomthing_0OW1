...
@Dark.on(admin_cmd(pattern="actiondp ?(.*)"))
...
async def autopic(event):
    while True:
        ...
        img = Image.open(photo)
        drawn_text = ImageDraw.Draw(img)
        fnt = ImageFont.truetype(FONT_FILE_TO_USEs, 30)
        drawn_text.text((300, 450), current_time, font=fnt, fill=(255, 255, 255))
        img.save(photo)
        file = await event.client.upload_file(photo)  # pylint:disable=E0602
        try:
...
@Dark.on(admin_cmd(pattern="rpc ?(.*)"))
async def autopic(event):
    while True:
        piclink = random.randint(0, len(TELEGRAPH_MEDIA_LINKSX) - 1)
        AUTOPP = TELEGRAPH_MEDIA_LINKSX[piclink]
        downloaded_file_name = "./DOWNLOADS/original_pic.png"
        downloader = SmartDL(AUTOPP, downloaded_file_name, progress_bar=True)
        downloader.start(blocking=False)
        photo = "photo_pfp.png"
...
CmdHelp("actiondp").add_command(
      ...
      'rpc' ,  None, 'Starts autodp of random pic'
).add()
