...
@Dark.on(admin_cmd(pattern=r"ss(?: |$)(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"ss(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern=r"css ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"css ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
