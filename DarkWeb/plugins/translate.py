...
@Dark.on(admin_cmd(pattern="trt ?(.*)"))
@Dark.on(sudo_cmd(pattern="trt ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern=r"trc", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"trc", allow_sudo=True))
async def _(Dark):
    if Dark.fwd_from:
        return
