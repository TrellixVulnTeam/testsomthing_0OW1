...
from DarkWeb import ALIVE_NAME, BIO_MSG, CMD_HELP
from Dark.utils import admin_cmd, edit_or_reply, sudo_cmd
from DarkWeb.cmdhelp import CmdHelp

DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "LEGENDARY AF DarkWeb"
DEL_TIME_OUT = 60
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Dark User"


@Dark.on(admin_cmd(pattern="autoname"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="autobio"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    while True:
...
@Dark.on(admin_cmd(pattern="reserved", outgoing=True))
@Dark.on(sudo_cmd(pattern="reserved", allow_sudo=True))
