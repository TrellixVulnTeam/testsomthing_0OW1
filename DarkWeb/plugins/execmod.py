...
@Dark.on(admin_cmd(outgoing=True, pattern="pips(?: |$)(.*)"))
...
@Dark.on(admin_cmd(pattern="suicide$"))
async def _(event):
    if event.fwd_from:
        return
    ...
    o = stdout.decode()
    OUTPUT = f"**[DarkWeb's](tg://need_update_for_some_feature/) SUICIDE BOMB:**\n{o}"
...
@Dark.on(admin_cmd(pattern="date$"))
async def _(event):
    if event.fwd_from:
        return
    PROCESS_RUN_TIME = 100
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[DarkWeb's](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await borg.send_file(
...
@Dark.on(admin_cmd(pattern="speed$"))
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[DarkWeb's](tg://need_update_for_some_feature/) , Server Speed Calculated:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await borg.send_file(
