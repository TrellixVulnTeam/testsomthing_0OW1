...
@Dark.on(admin_cmd(pattern="logo (.*)"))
@Dark.on(sudo_cmd(pattern="logo (.*)", allow_sudo=True))
async def lg1(Dark):
    event = await edit_or_reply(Dark, "`Processing.....`")
    fnt = await get_font_file(Dark.client, "@VegaFonts")
    if Dark.reply_to_msg_id:
        rply = await Dark.get_reply_message()
    ...
    text = Dark.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
