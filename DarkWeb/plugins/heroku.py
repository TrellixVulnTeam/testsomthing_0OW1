...
@Dark.on(admin_cmd(pattern="usage(?: |$)", outgoing=True))
@Dark.on(sudo_cmd(pattern="usage(?: |$)", allow_sudo=True))
...
@Dark.on(admin_cmd(pattern="logs$", outgoing=True))
async def _(dyno):
    if dyno.fwd_from:
        return
    try:
        ...
        return await dyno.reply(
            " Please make sure your Heroku API Key, Your App name are configured correctly in the heroku\n\n[Visit Support Group For Help](https://t.me/DarkWeb_Chit_Chat)"
        )
