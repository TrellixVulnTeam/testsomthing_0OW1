...
from DarkWeb import CmdHelp, bot as DarkWeb, ALIVE_NAME
from Dark.utils import admin_cmd, sudo_cmd, edit_or_reply as eor
...
@DarkWeb.on(admin_cmd(pattern="hvd$"))
@DarkWeb.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@DarkWeb.on(admin_cmd(pattern="vday$"))
@DarkWeb.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
