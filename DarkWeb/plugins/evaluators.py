...
@Dark.on(admin_cmd(pattern="exec(?: |$|\n)(.*)", command="exec"))
@Dark.on(sudo_cmd(pattern="exec(?: |$|\n)(.*)", command="exec", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="eval(?: |$|\n)(.*)", command="eval"))
@Dark.on(sudo_cmd(pattern="eval(?: |$|\n)(.*)", command="eval", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cmd = "".join(event.text.split(maxsplit=1)[1:])
...
@Dark.on(admin_cmd(pattern="bash ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="bash ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on DarkWeb server and shows details'
).add_command(
