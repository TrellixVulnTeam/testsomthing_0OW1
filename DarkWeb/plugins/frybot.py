...
@Dark.on(admin_cmd(pattern="frybot$"))
@Dark.on(sudo_cmd(pattern="frybot$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="deepfry(?: |$)(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="deepfry(?: |$)(.*)", allow_sudo=True))
