...
from Dark.utils import admin_cmd, progress
...
@Dark.on(admin_cmd(pattern="compress"))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".7z",
                caption="7z archived By DarkWeb",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
...
@Dark.on(admin_cmd(pattern="unzip"))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.edit("Processing ...")
...
@Dark.on(admin_cmd(pattern="unrar"))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.edit("Processing ...")
...
@Dark.on(admin_cmd(pattern="untar"))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.edit("Processing ...")
