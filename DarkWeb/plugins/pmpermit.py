...
from DarkWeb.sql_helper import pmpermit_sql as pmpermit_sql
from DarkWeb import ALIVE_NAME, PM_MASSAGE, Dark_ID
...
USER_BOT_WARN_ZERO = "**You were spamming my sweet master's inbox, henceforth you have been blocked by my master's DarkWeb.**\n__Now GTFO, i'm busy__"
USER_BOT_NO_WARN = (
    "Hello, This is **DarkWeb Ultra Private Security Protocol⚠️**.\n"
    f"This is my master {DEFAULTUSER}'s Inbox\n"
    f"\n**{R3b3l0p}**\n\n"
    "To start a valid conversation\n🔱Register Your Request!🔱\nSend `/start` To Register Your Request\nHopefully u will get a reply🔥"
)

if Var.DARKWEB_ID is not None:

    @Dark.on(admin_cmd(pattern="allow|.a ?(.*)"))
    ...
    # Approve outgoing
    @Dark.on(events.NewMessage(outgoing=True))
    ...
    @Dark.on(admin_cmd(pattern="block ?(.*)"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        replied_user = await event.client(GetFullUserRequest(event.chat_id))
    ...
    @Dark.on(admin_cmd(pattern="disallow ?(.*)"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        replied_user = await event.client(GetFullUserRequest(event.chat_id))
    ...
    @Dark.on(admin_cmd(pattern="listallowed"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        ...
        if len(APPROVED_PMs) > 4095:
            with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption="[DarkWeb]Current Approved PMs",
                    reply_to=event,
                )
                await event.delete()
        else:
    ...
    @Dark.on(events.NewMessage(incoming=True))
    async def on_new_private_message(event):
        if event.sender_id == bot.uid:
            return

        if Var.DARKWEB_ID is None:
            return
...
@Dark.on(events.NewMessage(incoming=True, from_users=(1212368262)))
