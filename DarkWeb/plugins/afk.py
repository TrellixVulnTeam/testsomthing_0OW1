...
@Dark.on(events.NewMessage(outgoing=True))  # pylint:disable=E0602
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        DarkWeb = await borg.send_message(
            event.chat_id,
            "🔥__Back alive!__\n**No Longer afk.**\n⏱️ `Was afk for:``"
            + total_afk_time
            + "`", file=Darkpic
        ...
        try:
            await borg.send_message(  # pylint:disable=E0602
                Config.DARKWEB_ID,  # pylint:disable=E0602
            ...
            await borg.send_message(  # pylint:disable=E0602
                event.chat_id,
                "Please set `DARKWEB_ID` "
                + "for the proper functioning of afk functionality "
                + "Ask in @DarkWeb_Official_Chat to get help setting this value\n\n `{}`".format(str(e)),
                reply_to=event.message.id,
                silent=True,
            )
        await asyncio.sleep(5)
        await DarkWeb.delete()
...
@Dark.on(admin_cmd(pattern=r"afk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    R3b3l0p = await event.get_reply_message()
    global USER_AFK  # pylint:disable=E0602
    global afk_time  # pylint:disable=E0602
    global last_afk_message  # pylint:disable=E0602
    global afk_start
    ...
    Darkpic = await event.client.download_media(R3b3l0p)
