...
@Dark.on(admin_cmd(pattern=r"commit"))
@Dark.on(sudo_cmd(pattern=r"commit"))
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of DarkWeb`")
        return
    DarkWeb = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await DarkWeb.edit("Committing to Github....")
        await git_commit(downloaded_file_name, DarkWeb)


async def git_commit(file_name, DarkWeb):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await DarkWeb.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await DarkWeb.edit("Cannot Upload Plugin")
    else:
        return await DarkWeb.edit("`Committed Suicide`")
