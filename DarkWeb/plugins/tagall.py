...
@Dark.on(admin_cmd(pattern=r"tagall (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"tagall (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern=r"admins", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"admins", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
