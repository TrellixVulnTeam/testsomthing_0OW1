...
import magic
import requests
from Dark.utils import admin_cmd, progress, sudo_cmd, edit_or_reply
from DarkWeb.cmdhelp import CmdHelp


@Dark.on(admin_cmd(pattern="verystream ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="verystream ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
