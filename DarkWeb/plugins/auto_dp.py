...
async def animeppbat():
    ...
    if not os.path.exists("f.ttf"):
        urllib.request.urlretrieve(
            "https://github.com/Dark6969/mym/raw/master/Dark-robot-Regular.ttf",
            "f.ttf",
        )
...
@Dark.on(admin_cmd(pattern="batmandp$"))
...
@Dark.on(admin_cmd(pattern="thordp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="actressdp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="animedp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="avengersdp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="gamerdp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="hackerdp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="spacedp$"))
async def main(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="wallpapers$"))
async def main(event):
    if event.fwd_from:
        return
