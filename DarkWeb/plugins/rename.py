...
@Dark.on(admin_cmd(pattern="rename (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="rename (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="rnupload (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="rnupload (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    thumb = None
...
@Dark.on(admin_cmd(pattern="rnsupload (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="rnsupload (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    await edit_or_reply(event, 
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await borg.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by [DarkWeb](https://t.me/DarkWeb_Chit_Chat)",
                    force_document=False,
                    allow_cache=False,
                    reply_to=event.message.id,
                    attributes=[
