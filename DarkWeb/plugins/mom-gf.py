...
@Dark.on(admin_cmd(pattern=r"gf$", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"gf$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern=r"mom$", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"mom$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 5
