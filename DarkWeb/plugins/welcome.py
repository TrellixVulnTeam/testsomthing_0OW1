...
from DarkWeb.sql_helper.welcome_sql import (
    add_welcome_setting,
...
@Dark.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
...
@Dark.on(admin_cmd(pattern="savewelcome", outgoing=True))  # pylint:disable=E0602
@Dark.on(sudo_cmd(pattern="savewelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="clearwelcome", outgoing=True)) # pylint:disable=E0602
@Dark.on(sudo_cmd(pattern="clearwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
@Dark.on(admin_cmd(pattern="listwelcome", outgoing=True)) # pylint:disable=E0602
@Dark.on(sudo_cmd(pattern="listwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
CmdHelp("welcome").add_command(
  "listwelcome", None, "Gets the saved welcome message of DarkWeb"
).add_command(
