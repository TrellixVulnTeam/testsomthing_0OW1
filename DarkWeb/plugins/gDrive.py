...
@Dark.on(admin_cmd(pattern=r"ugdrive ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"ugdrive ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern=r"drivesch ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"drivesch ?(.*)", allow_sudo=True))
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@Dark.on(admin_cmd(pattern=r"gdrivedir ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"gdrivedir ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
    ...
    if Var.DARKWEB_ID is None:
        await edit_or_reply(event, 
            "Please set the required environment variable `DARKWEB_ID` for this plugin to work"
        )
        return
    input_str = event.pattern_match.group(1)
...
async def create_token_file(token_file, event):
    ...
    async with bot.conversation(int(Var.DARKWEB_ID)) as conv:
        await conv.send_message(
            f"Go to the following link in your browser: {authorize_url} and reply the code"
        )
        response = conv.wait_event(
            events.NewMessage(outgoing=True, chats=int(Var.DARKWEB_ID))
        )
...
@Dark.on(admin_cmd(pattern=r"gfolder ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern=r"gfolder ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
