...
@Dark.on(admin_cmd(pattern="help ?(.*)", outgoing=True))
async def yardim(event):
    ...
    if tgbotusername is not None or Dark_input == "text":
        results = await event.client.inline_query(tgbotusername, "@DarkWeb_SUPPORT")
...
@Dark.on(sudo_cmd(allow_sudo=True, pattern="help ?(.*)"))
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of DarkWeb\n\n"
        )
        Darkcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the DarkWeb are [here]({url})"
