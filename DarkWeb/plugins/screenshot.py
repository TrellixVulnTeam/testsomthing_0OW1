...
@Dark.on(admin_cmd(pattern="screenshot (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="screenshot (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    try:
        ...
        with io.BytesIO(im_png) as out_file:
            out_file.name = "DarkWeb.ScreenCapture.PNG"
...
CmdHelp("screenshot").add_command(
  "screenshot", "<link>", "Gives out the web screenshot of given link via Google Crome Bin in .png format", ".screenshot https://github.com/Dark725/DarkWeb"
).add()