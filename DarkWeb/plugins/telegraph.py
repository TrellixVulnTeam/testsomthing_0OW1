...
@Dark.on(admin_cmd(pattern="t(m|t) ?(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="t(m|t) ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if Config.DARKWEB_ID is None:
        await edit_or_reply(event, "Please set the required environment variable `DARKWEB_ID` for this plugin to work\n\nGo to [DarkWeb Chat Group](t.me/DarkWeb_SUPPORT) for assistance"
        )
        return
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await edit_or_reply(event, 
                   "✓ **YOUR FILE :-** https://telegra.ph{} \n✓ **Time Taken :-** `{}` secs \n✓ **By :- [{}](tg://user?id={})**".format(
                        media_urls[0], (ms + ms_two), Dark_NAME, R3b3l0p
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            await edit_or_reply(event, 
                  f"✓ **Pasted to** [telegraph]({himanshu}) \n✓ **Time Taken :-** `{ms}` secs\n✓** By :- **[{Dark_NAME}](tg://user?id={R3b3l0p})", link_preview=True)
    else:
        await edit_or_reply(event, 
