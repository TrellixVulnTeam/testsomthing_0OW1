...
from DarkWeb import CMD_HELP, bot
...
@Dark.on(admin_cmd(pattern="pbio (.*)"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="pname ((.|\n)*)"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="ppic"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(outgoing=True, pattern="username (.*)"))
...
@Dark.on(admin_cmd(outgoing=True, pattern="count$"))
...
@Dark.on(admin_cmd(outgoing=True, pattern=r"delpfp"))
...
@Dark.on(admin_cmd(pattern="myusernames$"))
async def _(event):
    if event.fwd_from:
        return
