...
@Dark.on(admin_cmd(pattern="(rmbg|srmbg) ?(.*)"))
@Dark.on(sudo_cmd(pattern="(rmbg|srmbg) ?(.*)", allow_sudo=True))
async def remove_background(event):
    ...
    remove_bg_image = "DarkWeb.png"
    if "image" in contentType:
        with open("DarkWeb.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await edit_or_reply(Darkevent, f"`{response.content.decode('UTF-8')}`")
        return
    if cmd == "srmbg":
        file = convert_tosticker(remove_bg_image, filename="DarkWeb.webp")
