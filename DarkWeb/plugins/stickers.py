...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_DarkWeb_{pack}"
        packnick = (
            f"{DarkWeb} Vol.{pack}"
            if DarkWeb
            else f"@{user.username}'s DarkWeb Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
...
@Dark.on(admin_cmd(outgoing=True, pattern="stkrinfo"))
@Dark.on(sudo_cmd(pattern="stkrinfo", allow_sudo=True))
