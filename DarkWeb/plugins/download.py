...
from Dark.utils import admin_cmd, humanbytes, progress, sudo_cmd
from DarkWeb.cmdhelp import CmdHelp

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Dark User"


@Dark.on(admin_cmd(pattern="download(?: |$)(.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="download(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
