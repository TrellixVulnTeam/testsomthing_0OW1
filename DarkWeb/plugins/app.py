"""Fetch App Details from Playstore. @DarkWeb_Support
...
@Dark.on(admin_cmd(pattern="app (.*)"))
@Dark.on(sudo_cmd(pattern="app (.*)", allow_sudo=True))
async def apk(event):
    if event.fwd_from:
        return
...
@Dark.on(admin_cmd(pattern="appr (.*)"))
@Dark.on(sudo_cmd(pattern="appr (.*)", allow_sudo=True))
...
@Dark.on(admin_cmd(pattern="mods ?(.*)"))
async def mod(event):
    if event.fwd_from:
        return
