...
@Dark.on(admin_cmd(pattern="fake ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    await event.delete()
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1212368262:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 99$ to my master__ [Himanshu](https://t.me/R3b3l0p) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
