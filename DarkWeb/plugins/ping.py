...
@Dark.on(admin_cmd(pattern=f"hbping$", outgoing=True))
@Dark.on(sudo_cmd(pattern=f"hbping$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@Dark.on(admin_cmd(pattern="ping$", outgoing=True))
@Dark.on(sudo_cmd(pattern="ping$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
    ...
    ms = (end - start).microseconds / 1000
    if Dark_IMG:
        Dark_caption = f"╰•★★  ℘ơŋɠ ★★•╯\n\n    ⚘  `{ms}`\n    ⚘  Oɯɳҽɾ [{DEFAULTUSER}](tg://user?id={R3b3l0p})"
