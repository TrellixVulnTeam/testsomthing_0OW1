...
async def iphonex(text):
    ...
    R3b3l0p = r.get("message")
    Darkurl = url(R3b3l0p)
    if not Darkurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(R3b3l0p).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    Dark = await sticcers[0].click("me", hide_via=True)
    if Dark:
        await bot.send_file(int(chat_id), Dark, reply_to=reply_to_id)
        await Dark.delete()
