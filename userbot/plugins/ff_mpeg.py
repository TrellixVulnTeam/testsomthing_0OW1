...
from Dark.utils import admin_cmd, sudo_cmd, progress
from DarkWeb import CMD_HELP
from DarkWeb.cmdhelp import CmdHelp

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./downloads/DarkWeb.media.ffmpeg"
...
@Dark.on(admin_cmd(pattern="ffmpegsave$"))
@Dark.on(sudo_cmd(pattern="ffmpegsave$", allow_sudo=True))
...
@Dark.on(admin_cmd(pattern="vtrim"))
@Dark.on(sudo_cmd(pattern="vtrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
...
@Dark.on(admin_cmd(pattern="atrim"))
@Dark.on(sudo_cmd(pattern="atrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
...
@Dark.on(admin_cmd(pattern="ffmpegclear$"))
@Dark.on(sudo_cmd(pattern="ffmpegclear$", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
