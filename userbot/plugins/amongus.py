...
@Dark.on(admin_cmd(pattern="imp(|n) (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="imp(|n) (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    R3b3l0p = bot.uid
    USERNAME = f"tg://user?id={R3b3l0p}"
...
@Dark.on(admin_cmd(pattern="timp(|n) (.*)", outgoing=True))
@Dark.on(sudo_cmd(pattern="timp(|n) (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    name = event.pattern_match.group(2)
