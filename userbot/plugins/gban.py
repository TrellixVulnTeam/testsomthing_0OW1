...
async def gban(userbot):
    ...
    try:
        ...
        return await REBELBOT.edit("**Something W3NT Wrong 🤔**")
    if user:
        ...
        await REBELBOT.edit("`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await REBELBOT.edit("**Error! User already gbanned.**")
    except:
        pass
    return await REBELBOT.edit(
...
async def handler(h1m4n5hu0p):
    if not h1m4n5hu0p.user_joined and not h1m4n5hu0p.user_added:
        return
    try:
